...
export default function ProblemsPage() {
  ...
  async function submitAnswer(problem: Problem) {
    ...
    const isCorrect = Math.abs(num - problem.answer) < 0.0001;
  ...
  const correctCount = [...userAnswers.values()].filter(
    (a) => a.is_correct
  ).length;
        ...
        <div className="fade-up" style={{ marginBottom: "2rem" }}>
          ...
          >
            <span style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
              ทำถูกแล้ว{" "}
              <strong style={{ color: "var(--green)" }}>{correctCount}</strong>{" "}
              / {totalCount} ข้อ
            </span>
            <div
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {ans && !ans.is_correct && (
                    <p style={{ fontSize: "0.78rem", color: "var(--red)", marginTop: "0.2rem" }}>
                      คำตอบล่าสุด: {ans.answer_given} (ลองใหม่ครั้งที่ {ans.attempt_count})
                    </p>
                  )}
                  {ans && ans.is_correct && (
